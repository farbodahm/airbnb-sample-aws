...
class GlueCrawlerService(Construct):
    ...
    def __init__(self, scope: Construct, id: str,
                 ...
                 csv_files_path: str = None,
                ):
        """
        Parameters:
        target_bucket_arn (str): ARN of S3 bucket that contains CSVs.
        csv_files_path (str): If target CSVs are in special path in given bucket, provide it here.
        Notes:
        * Should get S3 bucket ARN instead of it's instance, otherwise it will be a cyclic reference.
        ...
        # IAM role associated with Glue Crawler
        glue_role = iam.Role(
            self,
            'airbnb-glue-role',
        ...
        # Glue Database to store CSVs metadata extracted by Crawler
        ...
        # Make file path valid for S3TargetProperty input
        if csv_files_path and csv_files_path[-1] != '/':
            csv_files_path += '/'

        s3_targets = glue.CfnCrawler.S3TargetProperty(
            path=f'{target_bucket.bucket_name}/{csv_files_path}'
        )

        # Glue Crawler to extract CSVs metadata
